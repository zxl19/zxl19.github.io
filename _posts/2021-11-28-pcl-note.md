...
5. 点云的初始化：
    ...
    // 方法二：通过points数据成员的函数成员添加点，需要手动维护width和height数据成员
    cloud2.points.push_back(cloud1.points[i]);
    cloud2.points.emplace_back(cloud1.points[i]);   // C++11引入，执行效率高
    // 以下两种方法均可以维护width和height数据成员
    // 方法一：
    cloud2.resize(cloud2.points.size());
    // 方法二：
    cloud2.width = static_cast<std::uint32_t>(cloud2.points.size());
    cloud2.height = 1;
    // 在不确定点云中是否存在Inf和NaN值时，建议将is_dense数据成员设置为false
    // 部分PCL函数会校验点云的is_dense数据成员，如果设置错误，会影响调用结果
...
12. 计算点云协方差：
    ...
    - `computeMeanAndCovarianceMatrix()`函数原型声明如下：
...
通过体素中心或者是随机取点，近似体素中点的质心，作为降采样后的点，精度低但是计算量小。如果点云在内存连续并且在空间中接近，则效果最佳，常结合NDT匹配算法使用。
...
random_sample.setSeed(42);       // 固定随机数种子，保证结果可复现
random_sample.setSample(10000);  // 如果采样数超过原始点云数，则返回全部点
...
extract.setNegative(false);  // 是否反选，默认为false，即提取inliers
...
11. [pcl::removeNaNFromPointCloud-代码先锋网](https://www.codeleading.com/article/26525841058/)
12. [pcl::getMinMax3D-CSDN博客](https://blog.csdn.net/qq_36501182/article/details/79005933)
13. [pcl::compute3DCentroid-CSDN博客](https://blog.csdn.net/u014072827/article/details/110746985)
14. [pcl::computeCovarianceMatrix-CSDN博客](https://blog.csdn.net/qq_45401419/article/details/127554396)
15. [pcl::computeMeanAndCovarianceMatrix-CSDN博客](https://blog.csdn.net/jacken123456/article/details/131181592)
16. [PCL中常用的高级采样方法-古月居的文章-知乎](https://zhuanlan.zhihu.com/p/699204931)
17. [pcl_viewer-CSDN博客](https://blog.csdn.net/qq_39779233/article/details/106985091)
